...
## 2. 会員サイト《 BOOSTER！応援共創ラボ 》 ← 2026-07-28 設置済み（トップページに独立コース）

サイト `94TOLagXD3lw` ＞ コース **「【予定】イベントスケジュール」**（`xpnIXMRLN1gu`／select値 `263330`・**公開・即時**）＞ レッスン「イベントスケジュール」（`28ePXIqCzoKG`・公開・即時）

- 会員のトップページにコースカードとして並ぶ（→ 受講する → カレンダー）。カード画像は `course-card-schedule.png`（1280×720）
- **6つのバンドル全部に追加済み**（Crewプラン／コミュニティプラン／実行委員＋企画ゼミ／講座のみ／共創マネタイズ実践講座用／出版販促参加特典）。**バンドルに入れないとトップページに出ない**のでコース新設時は必ずセットで行う
- 最初に「【はじめに】Boosterの歩き方」（`Hrc9moMmRPfv`）内に作ったレッスン `fqsZUi2R3SZG` は**下書きに戻して非表示**にしてある（削除はしていない）
- コースの表示順はドラッグ&ドロップのためMCP自動操作不可。今は一覧の最後。上に動かすなら手動
